...
function gameSearch() {
    ...
    let rex_rule = /[ \-\.?:\\\/\_\'\*]+/g;
    let value1 = keywords.replace(rex_rule, " ").trim().toLowerCase();
    value1 = value1.normalize('NFD').replace(/[\u0300-\u036f]/g, '');
    // console.log("value1 "+value1);
    return value1;
}

// th1(main): click input => t/d input
$('#game-search').on('input', function (event) {
    // console.log(1)
    if ($('#search-ajax').hasClass('hidden-search')) {
        $('#search-ajax').removeClass('hidden-search');
    }
    // when <input type="search"
    if (event.target.value === '') {
        $('#search-ajax').addClass('hidden-search');
    }
...
// th4: When clicking outside the search game: hidden #game-search
...
// th5: Click the input tab again: check if exist => show 
...
// get game ajax with keywords
function getGameAjax(kw) {
...
if (loader) {
    const ajax_games = document.getElementById("ajax-append");
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response !== '') {
                ...
                $("#post_item_ajax").append(response);
            }
